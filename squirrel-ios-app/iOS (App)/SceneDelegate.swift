    ...
    func startLocalServer() {
        ...
        func childChannelInitializer(channel: Channel) -> EventLoopFuture<Void> {
            return channel.pipeline.configureHTTPServerPipeline(withErrorHandling: true).flatMap {
                channel.pipeline.addHandler(HTTPHandler(fileIO: fileIO, htdocsPath: defaultHtdocs))
            }
        }
...
//                try socketBootstrap.bind(host: defaultHost, port: defaultPort).wait()
//            }()

            let channel = try socketBootstrap.bind(host: defaultHost, port: defaultPort).wait()

            guard let channelLocalAddress = channel.localAddress else {
                fatalError("Address was unable to bind. Please check that the socket was not closed or that the address family was understood.")
            }
            let localAddress = "\(channelLocalAddress)"

            NSLog("tzmax: 本地服务器启动成功。\(localAddress)")
            ...
            NSLog("tzmax: 本地服务器启动失败。\(exception)")
        }
    }
}
...
private final class HTTPHandler: ChannelInboundHandler {
    ...
    func handleJustWrite(context: ChannelHandlerContext, request: HTTPServerRequestPart, statusCode: HTTPResponseStatus = .ok, string: String, trailer: (String, String)? = nil, delay: TimeAmount = .nanoseconds(0)) {
        ...
        case .end:
            self.state.requestComplete()
            context.eventLoop.scheduleTask(in: delay) { () in
